# Análise de Tempo de Permanência em Redes Sociais
...
Para a construção da visualização final, foram seguidas as seguintes etapas técnicas:
1. Tabulação de Dados:
   Os dados brutos foram organizados em classes de intervalo (ex: 0 |-- 1, 1 |-- 2) para agrupar as respostas.
   
2. Cálculo de Frequência (fi):
   Contabilizou-se quantas pessoas pertenciam a cada classe, identificando que a maior concentração está no intervalo $2 \vdash 3$, com 16 pessoas.

3. Construção do Histograma:
   As barras foram configuradas com Largura da Lacuna em 0% para representar a continuidade dos intervalos de tempo.

4. Polígono de Frequência (Tendência de Uso):
   Foi sobreposta uma série de "Linha com Marcadores" conectando os pontos médios de cada barra para visualizar a curva de distribuição.
